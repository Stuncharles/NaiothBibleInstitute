...
export default function RegistrationForm() {
  ...
  // Real-time live Firebase synchronization for active registration status
  useEffect(() => {
    if (!successData?.id) return;
    const unsub = registrationService.subscribeToSingleRegistration(successData.id, (updated) => {
      if (updated) {
  ...
  const onSubmitHandler = async (e: FormEvent) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        try {
          localStorage.setItem("nbi_last_registration", JSON.stringify(response.data));
          window.dispatchEvent(new Event("nbi_registration_added"));
        } catch (_) {}
        reset();
        setStep(1);
      } else {
  ...
  const handleTrackSubmit = async (e: FormEvent) => {
    e.preventDefault();
    if (!trackQuery.trim()) {
      setTrackError("Please enter your Registration ID or Email address.");
      return;
    }
    setTrackLoading(true);
    setTrackError(null);
    try {
      const res = await registrationService.trackRegistration(trackQuery.trim());
      if (res.success && res.data) {
        setSuccessData(res.data);
      } else {
        setTrackError((res as any).message || "Registration record not found. Please verify your Registration ID or Email.");
      }
    } catch (err: any) {
      setTrackError(err.response?.data?.message || "Could not find registration record. Please try again.");
    } finally {
  ...
  return (
    <section id="register-section" className="py-24 bg-brand-blue/5 dark:bg-slate-950 relative overflow-hidden">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 relative z-10 w-full">
        ...
        {/* Title */}
        <div className="text-center max-w-xl mx-auto mb-8 space-y-3 print:hidden">
          ...
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 font-bold leading-relaxed">
            Submit your application or look up an existing registration record using your Registration ID or Email address.
          </p>

          {/* Mode Switcher Tabs */}
          <div className="flex justify-center p-1.5 bg-slate-100 dark:bg-slate-800/80 rounded-2xl max-w-md mx-auto mt-4 border border-slate-200/50 dark:border-slate-700/50">
            <button
              type="button"
              onClick={() => {
                setActiveTab("apply");
              ...
              className={`flex-1 py-2.5 px-4 rounded-xl text-xs sm:text-sm font-extrabold transition-all cursor-pointer ${
                activeTab === "apply"
                  ? "bg-white dark:bg-slate-900 text-brand-blue dark:text-brand-gold shadow-sm"
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-xl border border-slate-200/60 dark:border-slate-800/80 p-8 sm:p-10 relative">
          ...
          <AnimatePresence mode="wait">
            {!successData ? (
              ...
              /* Track / Search Form View */
              <form onSubmit={handleTrackSubmit} className="space-y-6 py-4 print:hidden">
                <div className="text-center space-y-2 mb-6">
                  <div className="w-12 h-12 bg-brand-blue/10 dark:bg-brand-accent/20 text-brand-blue dark:text-brand-gold rounded-full flex items-center justify-center mx-auto">
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 font-semibold">
                    Enter your Registration ID (e.g. NBI2026-0003) or Email Address to retrieve your confirmation slip and status.
                  </p>
                </div>
                ...
                <div className="space-y-2 max-w-md mx-auto">
                  ...
                  <input
                    ...
                    placeholder="e.g. NBI2026-0003 or stuncharles@gmail.com"
                    className="w-full px-4 py-3.5 bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-brand-blue dark:focus:ring-brand-gold text-slate-800 dark:text-white"
                  />
                </div>

                {trackError && (
                  <div className="p-4 bg-red-50 dark:bg-red-950/20 text-red-600 dark:text-red-400 border border-red-100 dark:border-red-900/30 rounded-2xl text-xs font-bold flex items-center gap-2 max-w-md mx-auto">
                ...
                <div className="flex justify-center pt-2">
                  <button
                    ...
                    className="px-8 py-3.5 bg-brand-blue hover:bg-brand-accent disabled:bg-brand-blue/50 text-white font-extrabold rounded-xl flex items-center space-x-2 shadow-md transition-colors cursor-pointer"
                  >
                    {trackLoading ? (
                      <>
                        <Loader2 className="w-4 h-4 animate-spin" />
                      ...
                      <>
                        <CheckCircle2 className="w-4 h-4" />
                        <span>View My Registration Record</span>
                      </>
                    )}
                  </button>
                </div>
              ...
              >
                ...
                <div className="p-6 sm:p-8 rounded-3xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 space-y-6 max-w-lg mx-auto text-left">
                  ...
                  {successData && (
                    <div className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-brand-blue/20 dark:border-brand-gold/20 space-y-3">
                      <div className="flex items-center justify-between pb-2 border-b border-slate-100 dark:border-slate-800">
                        <div>
                          <span className="text-[10px] font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-widest block">REGISTRATION ID</span>
                          <span className="text-base font-black text-brand-blue dark:text-brand-gold font-mono">{successData.id}</span>
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs font-semibold">
                        <div>
                          ...
                          <span className="text-slate-800 dark:text-white font-bold">{successData.fullName}</span>
                        </div>
                        <div>
                          <span className="text-slate-400 text-[10px] block">Email</span>
                          <span className="text-slate-800 dark:text-white font-bold truncate block">{successData.email}</span>
                        </div>
                        <div>
                          <span className="text-slate-400 text-[10px] block">Phone Number</span>
                          <span className="text-slate-800 dark:text-white font-bold">{successData.phoneNumber || "N/A"}</span>
                        </div>
                        <div>
                          <span className="text-slate-400 text-[10px] block">Church</span>
                          <span className="text-slate-800 dark:text-white font-bold truncate block">{successData.church || "N/A"}</span>
                        </div>
                      </div>

                      {successData.qrCode && (
                        <div className="pt-2 flex flex-col items-center justify-center border-t border-slate-100 dark:border-slate-800">
                          <img src={successData.qrCode} alt="Registration QR Code" className="w-24 h-24 rounded-lg shadow-xs" />
                          <span className="text-[10px] text-slate-400 font-bold mt-1">Official Verification QR Code</span>
                        </div>
                      )}
                    </div>
                  )}
